...
import { clear, fill, h, store } from "./ui.js";
...
async function route() {
  ...
  try { current = await routes[name]({ el: view, param, query, app }); }
  catch (e) { console.error(e); fill(view, h("div", { class: "errbox" }, `This view failed to load: ${e.message}`)); }
  window.scrollTo(0, 0);
}

window.addEventListener("hashchange", route);
if (!location.hash) location.replace("#/create");
api.settings().then((s) => { app.settings = s; if (s.theme && s.theme !== store.get("theme", "system")) applyTheme(s.theme); }).catch(() => {});
refreshStatus();
